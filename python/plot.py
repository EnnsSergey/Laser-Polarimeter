...
# Q
axes[1].errorbar(xs, dev_Q, yerr=err_Q, fmt='s-', capsize=3, 
                 color='red', linewidth=2, markersize=6)
axes[1].set_ylabel('Q_true - Q', fontsize=12)
axes[1].grid(True, alpha=0.3)

# beta
axes[2].errorbar(xs, dev_beta, yerr=err_beta, fmt='^-', capsize=3, 
                 color='green', linewidth=2, markersize=6)
axes[2].set_ylabel('beta_true - beta', fontsize=12)
axes[2].set_xlabel(r'$\sigma_{x^{\prime}},$ мкрд', fontsize=12)
axes[2].grid(True, alpha=0.3)

# Общий заголовок
title_line = r'Зависимость отклонений параметров от $\sigma_{x^{\prime}}$ при $\sigma_{y^{\prime}} = 50$ мкрд'
