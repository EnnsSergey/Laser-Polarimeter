...
На ускорительном комплексе ВЭПП-4M с детектором КЕДР проходит эксперимент по определению массы $\Upsilon$(1S)-мезона (состояние с наименьшей энергией и нулевым моментом импульса); эта
...
Лазерный поляриметр это устройство, измеряющее поляризацию пучка, лазерный луч светит на пучок а потом мы детектируем рассеянные $\gamma-$кванты. Сама поляризация определяется с помощью метода обратной свёртки с винеровской регуляризацией.
Несмотря на то, что абсолютное значение поляризации несущественно для МРД, тем не менее определение систематической погрешности используемой методики представляет собой научный интерес. Для этого необходимо изучить отклик детектора на известные параметры комптоновского рассеяния на пучке, для этого нужно создать Монте-Карло моделирование. Имеющееся моделирование уже неактуально для нынешней версии поляриметра. То есть, необходимо новое моделирование. 
...
При воздействии на пучок внешним электромагнитным
полем с частотой $\omega_d$, удовлетворяющей условию
\begin{equation}
...
(вертикальной) поляризации электронного пучка дифференциальное сечение рассеяния зависит от 
...
Таким образом, измерение поляризации пучка является ключевой операцией в МРД. Для моделирования работы поляриметра необходимо детальное понимание эффекта Комптона, чему посвящён следующий раздел.
\newpage
\section{Эффект Комптона}
В этом разделе будет рассмотрена теоретическая основа работы лазерного поляриметра — обратное комптоновское рассеяние. Будут выведены формулы для энергии рассеянных фотонов и, что наиболее важно, получено дифференциальное сечение этого процесса с учётом поляризации как электронов, так и фотонов. Полученное выражение ляжет в основу Монте-Карло генератора рассеяния.
...
\end{aligned}
\end{equation}
Формулы  \eqref{eq:final_cross_section} и \eqref{eq:compt_en} важнейшие для этой работы. Они описывает угловое и энергетическое распределение рассеянных фотонов и будут использованы далее для разработки эффективного алгоритма моделирования.
\newpage
...
\end{enumerate}
Такой подход позволяет получать более реалистичное распределение фотонов, учитывающее неидеальность пучка.
\newpage
\section{Физика взаимодействия излучения с веществом}
Прежде чем переходить к моделированию в Geant4, необходимо кратко описать ключевые физические процессы, которые определяют прохождение жёстких гамма-квантов и заряженных частиц через элементы установки.
...
Данные на детекторе можно приближённо представить свёрткой комптоновского сечения с некоторой функцией $B(x,y)$, вызывающей размывание. Оно происходит из-за разброса в пучке, взаимодействия $\gamma-$квантов с установкой, диффузии электронов в GEM детекторе.
\begin{equation}
...
На основе разработанного генератора в этом разделе строится полная модель экспериментальной установки в среде Geant4. Целью является получение реалистичного отклика детектора, который затем будет обработан по той же методике, что и реальные экспериментальные данные.
...
Далее нам нужно поставить в соответствие координатам номер пада детектора. 
...
\end{equation}
Вклад диффузии в общее уширение сигнала составляет около 20$\%$, что необходимо учесть в будущем при моделировании GEM детектора, но в текущей работе пренебрежём.

\section{Исследование}
На заключительном этапе работы собранная модель была использована для изучения того, как параметры пучка и лазера влияют на точность восстановления поляризации. 
...
\begin{figure}[H]
    ...
    \label{fig:placeholder}
\end{figure}
Мы видим, что подгонка поляризации имеет систематический сдвиг и он увеличивается с повышением порога для заряда на паде.
