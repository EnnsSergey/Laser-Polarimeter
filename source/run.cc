...
//	std::filesystem::path dir = "output_pb_"+ std::to_string(static_cast<int>(fParams.conv_thick));
	...
	std::filesystem::path charge_path = full_path / "charge.txt";
	outFile.open(charge_path, std::ios::app);
	for (int i : spectrum)
	{
		outFile << i << " ";
	}
